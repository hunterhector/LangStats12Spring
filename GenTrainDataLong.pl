...
sub Usage {
    my $err = shift;
    print "$err\n";
    print STDERR "Usage: <Filter Word File> <Example File> <Label File> <# of repeatitions> <Binary Lang Model> <Real Sentence File> <Out File> <Out Label File> <Add Len>\n";
    exit 1
}

my $FilterWordFile  = $ARGV[0] or Usage("Specify <Filter Word File>");
my $ExFile          = $ARGV[1] or Usage("Specify <Example File>");
my $LabelFile       = $ARGV[2] or Usage("Specify <Label File>");
my $NumRep          = $ARGV[3] or Usage("Specify <# of repetitions>");
my $BinLangModel    = $ARGV[4] or Usage("Specify <Binary Lang Model>");
my $RealSentFile    = $ARGV[5] or Usage("Specify <Real Sentence File>");
my $OutFile         = $ARGV[6] or Usage("Specify <Out File>");
my $OutLabelFile    = $ARGV[7] or Usage("Specify <Out Label File>");
my $AddLen          = $ARGV[8] or Usage("Sepcify <Add Length>");
...
print "Read docs #", scalar(@docs), " lab #", scalar(@labs), " # of words to keep: ".(keys(%FilterHash))."\n";
