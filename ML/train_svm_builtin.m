...
%%svm classifier        
        ...
        model = svmtrain(train_data, train_label, 'boxconstraint', C, 'kernel_function', 'rbf', 'rbf_sigma', para, 'options', options);
        %weight(i,:) = model{1}.w;
        ...
        Y_test = svmclassify(model, test_data);
...
model = svmtrain(data, label, 'boxconstraint', C, 'kernel_function', 'rbf', 'rbf_sigma', para, 'options', options);
...
for d=1:2
    ...
    Y_test = svmclassify(model, data_dev);
