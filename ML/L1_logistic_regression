...
for time = 1:T
    ...
    for i = 1:n_para
        svm_option = ['-c ',num2str(10^(i-n_para/2)),' -s 6']; % L1 logistic regression
%%svm classifier        
...
svm_option = ['-c ',num2str(para), ' -s 6'];

%%svm classifier        
model = {train(label,sparse(data), svm_option)};        
weight(i,:) = model{1}.w;
y = zeros(size(test_data,1),1);
[Y_test,precision,prob] = predict(label_dev, sparse(data_dev),model{1}, '-b 1');
